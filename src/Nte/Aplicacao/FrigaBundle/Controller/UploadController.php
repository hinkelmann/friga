...
class UploadController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        return $this->render('NteAplicacaoCadastrosBundle:frigaarquivo:index.html.twig', array(
            'arquivos' => $arquivos,
        ));
    }
    ...
    public function downloadArquivoHashAction(Request $request, $hash, $id)
    {
        ...
        } else if ($frigaArquivo->getTipo() == 99) {
            $response = new BinaryFileResponse($frigaArquivo->getPath());
            $response->setContentDisposition(ResponseHeaderBag::DISPOSITION_ATTACHMENT);
            return $response;
        } else {
    ...
    public function removeFileAction(Request $request, $id, $hash)
    {
        ...
        if ($arquivo) {
            try {
                if (count($frigaArquivo->findByHash($hash)) == 1 and file_exists($arquivo->getPath())) {
